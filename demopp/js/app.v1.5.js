...
const PROFILE_STORE = "secure-p2p-chat:profiles:v2";
...
function base64UrlDecode(value) {
  ...
  if (!/^[A-Za-z0-9_-]+$/.test(compact)) {
    throw new Error("Il codice contiene caratteri non validi. Copia solo il blocco che inizia con SP2P1.");
  }

  let base64 = compact.replaceAll("-", "+").replaceAll("_", "/");
...
function renderProfilesList() {
  ...
  if (!names.length) {
    els.profilesList.textContent = "Nessun profilo salvato per questa versione.";
    return;
  }
...
async function createProfile() {
  ...
  const profiles = getProfiles();
  if (profiles[username]) throw new Error("Esiste già un profilo con questo nome in questa versione.");
  ...
  const profile = {
    ...
    publicKeyBase64: bytesToBase64(publicSpki),
...
async function loginWithProfile(username, password) {
  ...
  if (!saved) throw new Error("Profilo non trovato in questa versione/browser.");

  const publicKeyBase64 = saved.publicKeyBase64 || (typeof saved.publicKey === "string" ? saved.publicKey : "");
  if (!publicKeyBase64) {
    throw new Error("Profilo locale creato da una vecchia build difettosa. Ricrea il profilo con questa versione.");
  }
  ...
  const publicRaw = base64ToBytes(publicKeyBase64);
  const publicCryptoKey = await importPublicKey(publicRaw);
  ...
  els.localFingerprint.textContent = `Tuo fingerprint: ${await fingerprintFromPublicKey(publicRaw)}`;
...
function extractSignalCode(value) {
  ...
  const match = compact.match(/SP2P1\.([A-Za-z0-9_-]+)/);
  if (!match) {
    throw new Error("Codice non riconosciuto: incolla il blocco che inizia con SP2P1.");
  }

  return SIGNAL_PREFIX + match[1];
}

function decodeSignal(value) {
  const token = extractSignalCode(value);
  const encodedPayload = token.slice(SIGNAL_PREFIX.length);

  try {
    const json = textDecoder.decode(base64UrlDecode(encodedPayload));
    const parsed = JSON.parse(json);

    if (!parsed || !parsed.type || !parsed.sdp) {
      throw new Error("Il codice è incompleto o non contiene un invito/risposta WebRTC.");
    }

    if (parsed.identityPublicKey && typeof parsed.identityPublicKey !== "string") {
      throw new Error("Il codice è stato creato da una vecchia build difettosa. Genera un nuovo invito con questa versione.");
    }

    return parsed;
  } catch (error) {
    if (
      error.message.includes("vecchia build") ||
      error.message.includes("incompleto") ||
      error.message.includes("caratteri non validi")
    ) {
      throw error;
    }

    throw new Error("Codice incollato non decodificabile. Copialo di nuovo partendo da SP2P1 fino all’ultimo carattere, senza modificarlo.");
  }
}
...
async function prepareAudioForConnection(pc) {
  ...
  if (!navigator.mediaDevices?.getUserMedia) {
    state.voiceMode = "listen";
    pc.addTransceiver("audio", { direction: "recvonly" });
    const message = "Microfono non disponibile in questo browser. Modalità ascolto: potrai sentire l’altro utente.";
  ...
  try {
    ...
    state.voiceMode = "speak";
    state.muted = false;
    els.muteBtn.disabled = false;
    els.muteBtn.textContent = "Mute";
    setVoiceStatus("Microfono attivo. Potrai parlare e ascoltare.");
    return "speak";
  } catch (error) {
    state.voiceMode = "listen";
    pc.addTransceiver("audio", { direction: "recvonly" });

    const raw = error?.message || error?.name || "errore sconosciuto";
    const reason =
      error?.name === "NotFoundError" || /not found|requested device/i.test(raw)
        ? "Nessun microfono trovato dal browser."
        : error?.name === "NotAllowedError"
          ? "Permesso microfono negato."
          : `Microfono non disponibile: ${raw}.`;

    const message = `${reason} Modalità ascolto attiva: potrai comunque sentire l’altro utente.`;
    setVoiceStatus(message);
    addSystemMessage(message);
    return "listen";
  }
...
function makePeerConnection() {
  ...
  pc.addEventListener("track", (event) => {
    ...
    if (stream) {
      ...
      const extra = state.voiceMode === "listen" ? " Sei in modalità ascolto." : "";
      setVoiceStatus(`Audio remoto ricevuto.${extra} Su Safari/iPhone potrebbe servire premere “Attiva audio remoto”.`);
...
function validateLocalProfileForPairing() {
  if (!state.profile) throw new Error("Fai login prima.");
  if (!window.RTCPeerConnection) throw new Error("WebRTC non disponibile in questo browser.");
  if (!window.isSecureContext) throw new Error("Apri l’app via HTTPS o localhost. GitHub Pages va bene; file:// no.");
  if (typeof state.profile.publicKeyBase64 !== "string" || !state.profile.publicKeyBase64.length) {
    throw new Error("Chiave pubblica locale non valida. Ricrea il profilo con questa versione.");
  }
}

async function createOffer() {
  validateLocalProfileForPairing();
  const pc = makePeerConnection();
  const voiceMode = await prepareAudioForConnection(pc);
  ...
  const signal = {
    ...
    fingerprint: await fingerprintFromPublicKey(state.profile.publicRaw),
    identityPublicKey: state.profile.publicKeyBase64,
    voiceMode,
...
async function setRemoteIdentity(signal) {
  if (!signal.identityPublicKey || typeof signal.identityPublicKey !== "string") {
    throw new Error("Il codice non contiene una chiave pubblica remota valida. Rigenera l’invito con questa versione.");
  }
  ...
  const voiceInfo = signal.voiceMode
    ? `<p class="muted">Voce remota: ${escapeHtml(signal.voiceMode === "speak" ? "parla e ascolta" : signal.voiceMode === "listen" ? "solo ascolto" : "disattivata")}</p>`
    : "";
...
function toggleMute() {
  ...
  setVoiceStatus(state.muted ? "Microfono in mute. Continui ad ascoltare." : "Microfono attivo.");
}

async function playRemoteAudio() {
  if (!els.remoteAudio.srcObject) {
    setVoiceStatus("Non ho ancora ricevuto audio remoto.");
    return;
  }
  try {
    await els.remoteAudio.play();
    setVoiceStatus(state.voiceMode === "listen" ? "Audio remoto attivo. Sei in modalità ascolto." : "Audio remoto attivo.");
  } catch (error) {
...
async function updateMicrophoneHint() {
  ...
  try {
    ...
    if (!audioInputs.length) {
      setVoiceStatus("Nessun microfono rilevato. Puoi comunque abilitare la voce in modalità ascolto.");
    }
  } catch {
    // Alcuni browser nascondono i dispositivi prima del permesso: non è critico.
  }
}
  ...
  els.copyOutgoingBtn.addEventListener("click", async () => {
    ...
    try {
      if (navigator.clipboard?.writeText) {
        ...
        els.outgoingSignal.focus();
        els.outgoingSignal.select();
      ...
      els.outgoingSignal.focus();
      els.outgoingSignal.select();
      setStatus("Copia manualmente il codice selezionato.", "warn");
    }
  });
...
function init() {
  console.info("Secure P2P Chat v1.5 loaded");
