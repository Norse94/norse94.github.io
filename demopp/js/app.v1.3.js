...
function extractSignalCode(value) {
  ...
  const match = cleaned.match(/SP2P1\.([A-Za-z0-9_-]+)/);
  if (!match) {
...
function decodeSignal(value) {
  ...
  try {
    ...
    if (parsed.identityPublicKey && typeof parsed.identityPublicKey !== "string") {
      throw new Error("Il codice è stato creato da una vecchia build difettosa. Genera un nuovo invito con la v1.3.");
    }
...
function init() {
  console.info("Secure P2P Chat v1.3 loaded");
